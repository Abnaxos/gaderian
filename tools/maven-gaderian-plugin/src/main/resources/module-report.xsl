...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:param name="javadoc.path" select="string('.')"/>
  ...
  <xsl:template match="/registry">
    ...
    <html>
      <head>
        <title>Gaderian Registry</title>
        <link rel="STYLESHEET" type="text/css" href="module-report.css"/>
      </head>
      <body>
        <div class="title">Gaderian Module Registry</div>
  ...
  <xsl:template name="head">
    ...
    <head>
      <link rel="stylesheet" type="text/css" href="../module-report.css"/>
      <title>Gaderian Registry - <xsl:value-of select="$title"/></title>
    </head>
  </xsl:template>
  <!-- output-description
  ...
  <xsl:template match="invoke-factory">
    <li> &lt;invoke-factory <span class="attribute">service-id</span>="<span class="attribute-value"><xsl:choose> <xsl:when test="@service-id"> <xsl:call-template name="link-to-service"> <xsl:with-param name="service" select="/registry/module/service-point[@id = current()/@service-id]"/> </xsl:call-template> </xsl:when> <xsl:otherwise> <a href="../service/gaderian.BuilderFactory.html">gaderian.BuilderFactory</a> </xsl:otherwise> </xsl:choose></span>" <span class="attribute">model</span>="<span
      class="attribute-value"><a href="../config/gaderian.ServiceModels.html"><xsl:choose> <xsl:when test="@model"> <xsl:value-of select="@model"/> </xsl:when> <xsl:otherwise>singleton</xsl:otherwise></xsl:choose></a></span>" <xsl:call-template name="raw-content"/> </li>
  </xsl:template>
  <!--
    == schema (detail-file)
  ...
  <xsl:template name="link-to-translator">
    ...
    <a href="../config/gaderian.Translators.html">
        ...
        <body>
          ...
          <table class="top-level-object" cellspacing="0">
            <tr>
              ...
              <th> Interface: </th>
              <td>
                <xsl:if test="boolean($javadoc.path)">
                  <a>
                    <xsl:attribute name="href">../<xsl:value-of select="$javadoc.path"/>/<xsl:value-of select="translate(@interface, '.', '/')"/>.html</xsl:attribute>
                    <xsl:value-of select="@interface"/>
                  </a>
                </xsl:if>
                <xsl:if test="not(boolean($javadoc.path))">
                  <xsl:value-of select="@interface"/>
                </xsl:if>
              </td>
            </tr>
